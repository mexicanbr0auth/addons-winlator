...
void VulkanRendererContext::createLogicalDevice() {
    ...
    // Mali G57 MC2 detection for tile-based optimization
    const char* devName = props.deviceName;
    if (strstr(devName, "Mali-G57") || strstr(devName, "mali-g57") ||
        strstr(devName, "Mali G57")) {
        isMaliGpu = true;
        __android_log_print(ANDROID_LOG_INFO, "Winlator_Renderer",
            "Mali G57 MC2 detected: tile-based optimizations enabled (device=%s)", devName);
    } else if (strstr(devName, "Mali")) {
        isMaliGpu = true;
        __android_log_print(ANDROID_LOG_INFO, "Winlator_Renderer",
            "Mali GPU detected: enabling tile-based optimizations (device=%s)", devName);
    }

    // For tile-based GPUs: prefer FIFO (VSync) to avoid tearing
    // Mali G57 handles FIFO well and it reduces thermal throttling
    if (isMaliGpu) {
        requestedPresentMode = VK_PRESENT_MODE_FIFO_KHR;
        __android_log_print(ANDROID_LOG_INFO, "Winlator_Renderer",
            "Tile-based GPU: using FIFO present mode for thermal efficiency");
    }
}
...
void VulkanRendererContext::createWinTexPool() {
    // Mali G57: use smaller pool for better memory efficiency on mobile
    uint32_t maxSets = isMaliGpu ? 64 : 129;
    VkDescriptorPoolSize ps{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, maxSets};
    VkDescriptorPoolCreateInfo ci{}; ci.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    ci.flags=VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT;
    ci.poolSizeCount=1; ci.pPoolSizes=&ps; ci.maxSets=maxSets;
...
void VulkanRendererContext::dumpRendererInfo() {
    ...
    __android_log_print(ANDROID_LOG_DEBUG,WLOG_TAG,
        "GPU Type: %s (tile-based=%d framesInFlight=%d)",
        isMaliGpu?"Mali":"Other",(int)isMaliGpu,(int)MAX_FRAMES_IN_FLIGHT);
